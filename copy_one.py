...
def copyFileOrSyncDir(srcfile, dstfile, afile, filename):
    # if it is a file simply copy it
    ...
    if os.path.isfile(srcfile):
        ...
        (filesCopied, bytesCopied) = syncDir(srcfile, dstfile, filename)
    return (filesCopied, bytesCopied)
        
def syncDir(dir1, dir2, filename):
    bytesCopied = 0
    filesCopied = 0
    ...
    for afile in files1:
        ...
        if isLinuxHiddenFile(srcfile):
            continue
        if files2.count(afile) == 0 or \
                (not os.path.isfile(srcfile)) or \
                ( isNewer(srcfile, dstfile) and \
                      not equalMD5(srcfile, dstfile)):
            # we use MD5 checksum to determine if the
            # file has changed.
            (subFiles, subBytes) = copyFileOrSyncDir(srcfile, dstfile, afile, filename)
...
if (__name__ == "__main__"):
    ...
    (filesCopied, bytesCopied) = syncDir(srcDir, dstDir, filename) 
